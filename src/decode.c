...
read_R2004_section_map(Bit_Chain *restrict dat, Dwg_Data *restrict dwg)
{
  ...
  //FIXME: check checksum (ODA p.27)
  {
    ...
    calc_checksum = dwg_section_page_checksum(0, dat, sizeof(dwg->r2004_header));
    LOG_TRACE("First r2004_header.checksum 0x%08x\n", calc_checksum);
    dwg->r2004_header.checksum = checksum;
    dat->byte = section_address; // before decomp
    calc_checksum = dwg_section_page_checksum(calc_checksum, dat, decomp_data_size);
    if (calc_checksum != checksum)
      LOG_INFO("Invalid 2004 System Section Page checksum 0x%08x != 0x%08x (TODO)\n",
                        ...
                        uint32_t decomp_data_size)
{
  ...
  dwg->header.section_info = (Dwg_Section_Info*)
    calloc(dwg->header.section_infohdr.num_desc, sizeof(Dwg_Section_Info));
...
decode_R2004_header(Bit_Chain *restrict file_dat, Dwg_Data *restrict dwg)
{
...

  {
    ...
    crc32 = _obj->crc32;
    _obj->crc32 = 0;
    calc_crc32 = bit_calc_CRC32(0, &decrypted_data[0], 0x72); //without the padding
    _obj->crc32 = crc32;
    if (calc_crc32 != crc32)
